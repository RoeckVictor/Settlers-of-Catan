...
namespace AI.Model
{
    class DifficultAI : AbstractAI
    {
        ...
        private (Coordinate, int) BestIntersection;
        ...
        private void EvaluateIntersectionsScores()
        {
            ...
            foreach (Coordinate c in IntersectionsDistances.Keys)
            {
                ...
                {
                    BestIntersection = (c, IntersectionsDistances[c]);
            ...
            for (int action = 0; ConstructionsNeeds.Count < 3;)
            {
                switch (bestActions[action].Item1)
                {
                    ...
                    case Action.Construct:
                        if (Intersections[BestIntersection.Item1].PlayerId == ai.Id)
                        {
        ...
        private void ConstructRoad()
        {
            ...
            if (ActionsScores[Action.Construct] > ActionsScores[Action.ExpandRoad])
            {
                ...
                intersection = BestIntersection.Item1;
        ...
        private void ConstructSettlement()
        {
            if (Intersections[BestIntersection.Item1].PlayerId == ai.Id)
            {
                GameView.Instance.OnConstruct(new ConstructEventArgs(game.Id, ConstructionType.CITY, BestIntersection.Item1, IsOnline));
            }
            else
            {
                GameView.Instance.OnConstruct(new ConstructEventArgs(game.Id, ConstructionType.SETTLEMENT, BestIntersection.Item1, IsOnline));
            }
        }
        ...
        */
        ...
        Coordinate NextRoad()
        {
            Coordinate source = BestIntersection.Item1, target = null;
        ...
        private BanditMoveEventArgs BanditMoveArgs()
        {
            ...
            if (IntersectionsDistances[BestIntersection.Item1] == 0)
            {
                if (Intersections[BestIntersection.Item1].Building == ConstructionType.SETTLEMENT)
                {
        ...
        private void PlaceFirstRoads()
        {
            ...
            EvaluateIntersectionsScores();
            GameView.Instance.OnInitialRoad(new InitialConstructEventArgs(game.Id, NextRoad(), IsOnline));
        }
        ...
        public override void Construct(IGame game)
        {
            ...
            while (constructionDone)
            {
                ...
                if (IntersectionsDistances[BestIntersection.Item1] == 0)
                {
                    ...
                    if (Intersections[BestIntersection.Item1].Building == ConstructionType.SETTLEMENT)
                    {
                        ...
                        if (Ressources[RessourceType.WHEAT] >= 2 && Ressources[RessourceType.ORE] >= 3)
                        {
                            ...
                            GameView.Instance.OnConstruct(new ConstructEventArgs(game.Id, ConstructionType.CITY, BestIntersection.Item1, IsOnline));
                            Constructions[Constructions.LastIndexOf((BestIntersection.Item1, ConstructionType.SETTLEMENT))] = (BestIntersection.Item1, ConstructionType.CITY);
                            Intersections[BestIntersection.Item1].AddBuilding(ConstructionType.CITY, ai.Id);
                            VictoryPoints += 1;
                    ...
                    else
                    {
                        ...
                        if (Ressources[RessourceType.LUMBER] >= 1 && Ressources[RessourceType.BRICK] >= 1 && Ressources[RessourceType.WOOL] >= 1 && Ressources[RessourceType.WHEAT] >= 1)
                        {
                            // C'est possible. L'IA construit.
                            GameView.Instance.OnConstruct(new ConstructEventArgs(game.Id, ConstructionType.SETTLEMENT, BestIntersection.Item1, IsOnline));
                            Constructions.Add((BestIntersection.Item1, ConstructionType.SETTLEMENT));
                            Intersections[BestIntersection.Item1].AddBuilding(ConstructionType.SETTLEMENT, ai.Id);
                            VictoryPoints += 1;
                            ...
                            if (Intersections[BestIntersection.Item1].Harbor != HarborType.NONE)
                            {
                                harbors.Add(Intersections[BestIntersection.Item1].Harbor);
                            }
                            constructionDone = true;
                        }
                        else
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        if (!DevelopmentCardUsed)
                        {
                            if (UsableCards[CardType.ROAD_BUILDING] > 0)
                            {
                                // Pour construire deux routes, la carte routes
                                Coordinate road1, road2;
                                (road1, road2) = NextRoads();
                                GameView.Instance.OnRoadConstructionCardUse(new RoadConstructionCardUseEventArgs(game.Id, road1, road2, IsOnline));
                                Edges[road1].AddBuilding(ConstructionType.ROAD, ai.Id);
                            ...
                            /*
                            else if (UsableCards[CardType.KNIGHT] > 0 && RessourcesCount() <= 7)
                            {
                                // Pour récupérer une ressource, la carte chevalier.
                            ...
                            */
                            else if (UsableCards[CardType.RESSOURCE_PAIR] > 0)
                            {
                                // Pour récupérer deux ressources, la carte invention.
                            ...
                            else if (UsableCards[CardType.RESSOURCE_MONOPOLY] > 0)
                            {
                                ...
                                RessourceType ressourceToMonopolize = RessourceType.NONE;
                                if (IntersectionsDistances[BestIntersection.Item1] >= 2)
                                {
                ...
                if (!constructionDone && !DevelopmentCardUsed)
                {
            ...
            if (!DevelopmentCardUsed && UsableCards[CardType.KNIGHT] > 0 && RessourcesCount() <= 5)
            {
